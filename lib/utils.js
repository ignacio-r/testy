...
// Returns the caller line of code of the method that is calling this method. The stack trace has this shape:
// 0 is string 'Error'
// 1 is the line where this function is being evaluated
// 2 is the line where the caller is calling this function
// 3 is the line of the caller's caller (what we want)
const callerLineOfCode = () => {
  const callerStackTraceLine = (new Error).stack.split("\n")[3] || '';
  const callerFileAndLineNumberSection = callerStackTraceLine.split("at")[1] || '';
  return callerFileAndLineNumberSection.trim();
}
